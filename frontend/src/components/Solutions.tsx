...
export const Solutions: React.FC = () => {
  ...
  return (
    <section aria-labelledby="solutions-title" className="min-h-screen bg-theme-base p-6 sm:p-10 relative overflow-hidden selection:bg-theme-accent selection:text-theme-text">
        ...
        <div className="flex flex-wrap gap-3">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                selectedCategory === category.id
                  ? "bg-gradient-to-r from-blue-600 to-blue-800 text-white border-transparent shadow-lg shadow-blue-500/20"
                  : "bg-white/60 backdrop-blur-md text-slate-700 border-slate-200 hover:bg-white hover:shadow-sm"
              }`}
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-8">
        {filteredSolutions.map((solution) => {
          ...
          return (
            <div
              key={solution.id}
              className="bg-white/70 backdrop-blur-md rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 hover:border-blue-200/50 transition-all duration-300 self-start relative z-10"
            >
              <div className="p-6">
      ...
      {/* Emergency Helpline */}
      <div className="mt-12 relative z-10 bg-gradient-to-r from-red-50/90 to-orange-50/90 backdrop-blur-md border border-red-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl p-6">
