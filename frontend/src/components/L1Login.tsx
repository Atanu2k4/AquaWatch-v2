...
import { ArrowLeft, UserCheck, Lock, Eye, EyeOff, Waves } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";

interface L1LoginProps {
  onBack: () => void;
  onSuccess: () => void;
  onRegister: () => void;
}

export const L1Login: React.FC<L1LoginProps> = ({ onBack, onSuccess, onRegister }) => {
  const [govtId, setGovtId] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const { l1Login } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await l1Login(govtId.trim(), password);
      onSuccess();
    } catch {
      setError("Invalid Govt ID or password. Ensure your account has been approved.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-[400px]">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white px-8 py-10 shadow-sm">
            <div className="mb-8 flex flex-col items-center text-center">
              ...
              <h1 className="text-[22px] font-bold text-slate-900">L1 Authority</h1>
              <p className="mt-1 text-[14px] text-slate-500">Field officer authenticated access</p>
            </div>
            ...
            <p className="mt-8 text-center text-[13px] text-slate-500">
              New officer?{" "}
              <button onClick={onRegister} className="font-medium text-blue-600 hover:underline">
                Register here
              </button>
            </p>
          </div>
