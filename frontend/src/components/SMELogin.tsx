...
export const SMELogin: React.FC<SMELoginProps> = ({ onBack, onSuccess, onRegister }) => {
  const [smeId, setSmeId] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const { smeLogin } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await smeLogin(smeId.trim(), password);
      onSuccess();
    } catch {
      setError("Invalid SME ID or password. Ensure your account has been approved by a State Admin.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-[400px]">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white px-8 py-10 shadow-sm">
            <div className="mb-8 flex flex-col items-center text-center">
              ...
              <h1 className="text-[22px] font-bold text-slate-900">SME / Authority</h1>
              <p className="mt-1 text-[14px] text-slate-500">
                Domain expert & department authority access
              </p>
            </div>
            ...
            <p className="mt-8 text-center text-[13px] text-slate-500">
              New expert?{" "}
              <button onClick={onRegister} className="font-medium text-violet-600 hover:underline">
                Register here
              </button>
            </p>
          </div>
